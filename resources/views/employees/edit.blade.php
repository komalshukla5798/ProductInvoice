    ...
    <div class="card-body">
        <form action="{{ route('employees.update', [$employee->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('phone_number') ? 'has-error' : '' }}">
                ...
                <input type="number" id="phone_number" name="phone_number" class="form-control" value="{{ old('phone_number', isset($employee) ? $employee->phone_number : '') }}" onkeypress="if(this.value.length > 10) return false; ">
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                @if(old('status') == '1')
                    checked="checked"
                @elseif((isset($employee) && $employee->status == '1') && 
                (old('status') !== '0')) 
                    checked="checked"
                @endif> Active
                ...
                @if(old('status') == '0')
                    checked="checked"
                @elseif((isset($employee) && $employee->status == '0') && 
                (( old('status') !== '1'))) 
                    checked="checked"
                @endif> Inactive
